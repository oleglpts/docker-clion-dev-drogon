# Debugging C++ in a Docker Container using Drogon development container with CLion

A base image for debug applications based on the Drogon framework (see https://drogon.org/)

> Note: Fork of [shuhaoliu/docker-clion-dev](https://github.com/shuhaoliu/docker-clion-dev)
...
0. Build the container.
    ...
    ```bash
    docker build -t docker_clion_dev:latest --build-arg alpine_version={apline_version} --build-arg drogon_version={drogon_version} \
--build-arg trantor_version={trantor_version} --build-arg revision={revision} .
    ```
...
0. Launch the container with `docker-compose`.
    ...
    docker compose up -d
    ```
    After this step, the container is running with an ssh server daemon. Clion will automatically run/test/debug via an ssh connection. The folder where `docker-compose.yml` locates will be the mapped to `/code` within the container.

0. Configure the container toolchain in Clion. Go to ***Settings/Preferences | Build, Execution, Deployment | Toolchains***, configure the container as a remote host. The username is `drogon`. The password is `pwd`. The completed configurations should be similar to the following: ![Toolchain configuration](configs/toolchain-config.png)
